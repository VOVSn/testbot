...
from settings import (
    ADMIN_USER_ID, ADMIN_USERNAME, # Still needed for initial bootstrap
...
async def _seed_initial_admin():
    ...
    if not ADMIN_USER_ID or not ADMIN_USERNAME:
        ...
        # Don't raise error, just log and skip if not configured
        return
    ...
    try:
        ...
        if existing_admin_count > 0:
            ...
            # Optional: Verify if the .env admin still has admin role
            # env_admin = await users_collection.find_one({'user_id': bootstrap_admin_id})
            # if not env_admin or env_admin.get('role') != 'admin':
            #     logger.warning(f"Admin specified in .env (ID: {bootstrap_admin_id}) does not currently have admin role in DB!")
            return
        # --- END MODIFIED LOGIC ---

        # No admins exist, proceed with bootstrap using .env details
        ...
        # Use update_one with upsert=True to create/update the bootstrap admin
        update_result = await users_collection.update_one(
            ...
            {'$set': {
             ...
             '$setOnInsert': { # Add date only if creating new
        ...
        if update_result.upserted_id:
            logger.info(f"Successfully bootstrapped initial admin user {ADMIN_USERNAME} (ID: {bootstrap_admin_id}).")
             ...
             # This case implies the user existed but wasn't admin (since count was 0)
             if update_result.modified_count:
                   logger.info(f"Successfully promoted existing user {ADMIN_USERNAME} (ID: {bootstrap_admin_id}) to admin during bootstrap.")
             else:
                   # Should not happen if count was 0 and matched>0 without modified>0
                    logger.warning("Admin bootstrap matched user but didn't modify role?")
        else:
             # Should ideally not happen with upsert=True unless DB error
             logger.warning("Admin bootstrap via update_one reported no match or upsert unexpectedly.")
...
async def _seed_tests():
    ...
    # --- Same logic as before: Add if not exists ---
    ...
    for filename in os.listdir(TESTS_SEED_FOLDER):
        if re.match(r'^test.*\.csv$', filename, re.IGNORECASE):
            ...
            # Check if test already exists (idempotency)
            ...
            try:
                with open(file_path, mode='r', encoding='utf-8') as csvfile:
                    # Robust parsing from upload_handler._handle_test_csv_upload
                    csv_reader = csv.reader(csvfile, delimiter=';')
                    line_num = 0
                    for row in csv_reader:
                        line_num += 1
                        if not row or not row[0].strip() or len(row) < 2: continue
                        question_text = row[0].strip()
                        correct_answer = row[1].strip()
                        if not correct_answer:
                             logger.warning(f"Row {line_num} in '{filename}' missing correct answer. Skipping row.")
                             continue
                ...
                if not questions_data:
                     logger.warning(f"No valid questions found in '{filename}'. Skipping test seed.")
                     continue
                ...
                test_doc = {
                    ...
                    'uploaded_by_user_id': 0, # Indicate seeded
                ...
                if insert_result.inserted_id:
                    logger.info(f"Successfully seeded test '{test_id}' from '{filename}'.")
                    seeded_count += 1
                else:
...
async def _seed_teachers():
    ...
    # Check if any teachers already exist
    ...
    # --- Same logic as before: Promote existing users if no teachers found ---
    ...
    try:
        with open(TEACHERS_SEED_FILE, mode='r', encoding='utf-8') as file:
            for line in file:
                ...
                # Find user and update role if they exist and aren't already teacher
                update_result = await users_collection.update_one(
                    {'username': username, 'role': {'$ne': 'teacher'}}, # Find by username, ensure not already teacher
                    {'$set': {'role': 'teacher'}}
                )
                if update_result.matched_count == 0:
                    # Check if they exist but are already teacher, or don't exist
                    user_exists = await users_collection.find_one({'username': username}, {'_id': 1, 'role': 1})
                    if user_exists and user_exists.get('role') == 'teacher':
                         logger.info(f"User @{username} from seed file is already a teacher.")
                    else:
                         logger.warning(f"Teacher seed file lists '@{username}', but user not found in DB (line {line_num}). User must /start first.")
                         not_found_count += 1
                elif update_result.modified_count == 1:
                    logger.info(f"Promoted user @{username} to teacher via seed file.")
                    promoted_count += 1
                # else matched but not modified - shouldn't happen with $ne condition

    except Exception as e:
        logger.exception(f"Error processing teacher seed file '{TEACHERS_SEED_FILE}': {e}")

    logger.info(f"Teacher seeding complete. Successfully promoted: {promoted_count}, Not found/Already Teacher: {not_found_count}.")


async def seed_initial_data():
    """Orchestrates the initial data seeding process based on settings."""
    logger.info("Running initial data seeding process...")
    await _seed_initial_admin() # Bootstrap first admin if needed
