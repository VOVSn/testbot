...
async def results_command(update: Update, context: CallbackContext) -> None:
    ...
    if is_teacher:
        ...
        logger.info(f"Found results for test {test_id}. Sending results to teacher {user_username}.")
        with open(grades_file, 'r', encoding='utf-8') as file:
            results = file.read()
        await update.message.reply_text(results)
    
    # Student case: no arguments allowed
    else:
        if context.args:
            logger.info(f"Ignoring arguments for /results command by student {user_username}.")
